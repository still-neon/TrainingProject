...
public abstract class DataParserTest {
	private static final String ERROR_MESSAGE_TEMPLATE = "Parsed %s value doesn't equal expected %s value. Actual: %s, Expected: %s";

	protected void checkDateParsing(DateToken dateToken, String dateValue) throws ParseException {
		Date date = new SimpleDateFormat(DataParser.getDATE_FORMAT()).parse(dateValue);
		assertTrue(String.format(ERROR_MESSAGE_TEMPLATE, "date", "date", dateToken.getDate(), date), dateToken.getDate().equals(date));
	}

	protected void checkNumberPairParsing(TextNumberPairToken textNumberPairToken, String text, Double number, String currency) {
