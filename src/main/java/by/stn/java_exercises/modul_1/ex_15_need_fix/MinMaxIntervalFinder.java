...
public class MinMaxIntervalFinder {
    ...
    public static int find(List<Integer> numbers) {
        ...
        int minNumber = numbers.get(0);//добавить метод min в классе MaxValueCalc
        int sum = 0;
    ...
    private static List<Integer> getNumberIndices(int number, List<Integer> numbers) {
        Set<Integer> indices = new TreeSet<>();//сортировать список методом sort
    ...
    private static List<Integer> getElementsIndices(List<Integer> number1Indices, List<Integer> number2Indices) {
        return getIndicesDifference(getFirstIndex(number1Indices), getLastIndex(number2Indices)) >=
                ...
                getIndices(getFirstIndex(number1Indices), getLastIndex(number2Indices)) :
                getIndices(getFirstIndex(number2Indices), getLastIndex(number1Indices));
    }

    private static int getIndicesDifference(int index1, int index2) {//класс интервал, возвращать только два индекса
        return Math.max(index1, index2) - Math.min(index1, index2);//модуль Math
    }

    private static List<Integer> getIndices(int index1, int index2) {
